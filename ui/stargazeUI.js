Game.stargazeUI = (function(){
    ...
    instance.initialise = function() {
        ...
        instance.rebirthTemplate = Handlebars.compile(
            ['<tr id="{{htmlId}}"><td>',
                '<h3 class="default btn-link" data-i18n="{{name}}">{{name}}</h3>',
                '<span>',
                    '<p>{{{desc}}}</p>',
                    '<p id="{{htmlId}}_cost"><span data-i18n="Costs:">Costs:</span> {{cost}} <span data-i18n="Dark Matter">Dark Matter</span></p>',
                '</span>',
                '<div id="{{htmlId}}_buy" onclick="Game.stargaze.upgrade(\'{{id}}\')" class="btn btn-warning"><span data-i18n="Rebirth">Rebirth</span></div>',
                '<br><br>',
                '</td></tr>'].join('\n'));

        instance.respecTemplate = Handlebars.compile(
            ['<tr id="{{htmlId}}"><td>',
                '<h3 class="default btn-link" data-i18n="{{name}}">{{name}}: <span id=\'respecCount\'>3</span></h3>',
                '<span>',
                    '<p>{{{desc}}}</p>',
                    '<p id="{{htmlId}}_cost"><span data-i18n="Costs:">Costs:</span> {{cost}} <span data-i18n="Dark Matter">Dark Matter</span></p>',
                '</span>',
                '<div id="{{htmlId}}_buy" onclick="Game.stargaze.upgrade(\'{{id}}\')" class="btn btn-warning"><span data-i18n="Respec">Respec</span></div>',
                '<br><br>',
                '</td></tr>'].join('\n'));

        instance.upgradeTemplate = Handlebars.compile(
            ['<tr id="{{htmlId}}"><td>',
                '<h3 class="default btn-link" data-i18n="{{name}}">{{name}}: <span id="{{htmlId}}Achieved"><span data-i18n="Dormant">Dormant</span></span></h3>',
                '<span>',
                    '<p>{{{desc}}}</p>',
                    '<p id="{{htmlId}}_cost"><span data-i18n="Costs:">Costs:</span> {{cost}} <span data-i18n="Dark Matter">Dark Matter</span></p>',
                    '<p id="{{htmlId}}_opinion"><span data-i18n="Improves relationship by">Improves relationship by</span> {{opinion}}</p>',
                '</span>',
                '<div id="{{htmlId}}_buy" onclick="Game.stargaze.upgrade(\'{{id}}\')" class="btn btn-default"><span data-i18n="Activate">Activate</span></div>',
                '<br><br>',
                '</td></tr>'].join('\n'));
