Game.achievements = (function() {
	...
	instance.getAchievementTitle = function(data, for_tooltip) {
		var lang = (typeof Game.lang !== 'undefined') ? Game.lang : null;
		var titleKey = data.title;
		var titleText = (lang) ? lang.get(titleKey) : titleKey;
		if(data.unlocked === data.brackets.length - 1) {
			var title = titleText.replace('%s', Game.settings.format(data.brackets[data.unlocked]));
			if(for_tooltip === true) {
				title += " (Completed)";
			}
			return title;
		} else {
			var title = titleText.replace('%s', Game.settings.format(data.brackets[data.unlocked+1]));
			if(for_tooltip === true) {
	...
	instance.update = function(delta) {
		for(var id in this.entries) {
			...
			if(data.unlocked < data.brackets.length - 1 && data.evaluator(bracket)) {
				var achTitle = (typeof Game.lang !== 'undefined') ? Game.lang.get('Achievement Reached') : 'Achievement Reached';
				Game.notifySuccess(achTitle, this.getAchievementTitle(data, false));
